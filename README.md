...
Two things to get right on Railway:
...
**Leaving Railway.** When/if that day comes, migration is trivial because the data is just a file:
...
**Keeping exit cheap.** Two habits make migration painless if you ever need it:
...
**If you ever outgrow SQLite** (you probably won't for this workload — see "Risks & mitigations"), `pgloader` migrates SQLite → Postgres in one command. The schema you'll have is simple enough that this is a half-day job, not a project.
